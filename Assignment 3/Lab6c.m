...
%% adjust final layer
net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{f*randn(6,6,10,2, 'single'), zeros(1, 2, 'single')}}, ...
                           'stride', 1, ...
                           'pad', 0, ...
                           'name', 'fc1') ;
...
feats = zeros(64,64,length(filesCat) + length(filesDog));
...
for i = 1:length(filesHorse) 
    ...
    labels(i + length(filesCat) + length(filesDog)) = 1;
end
...
for i = 1:length(filesHorse) 
    ...
    groundtruthLabel(i + length(filesCat) + length(filesDog)) = 1;
end
